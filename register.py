...
def mai(root):
    ...
    r1.Img2 = PhotoImage(file="elements/candidate.png")
    recruit2 = Button(r1, image=r1.Img2, border=0, bg="#05edFC",
                      relief="raised", activebackground="#05F6FD", command=lambda: candidate_regis(root))
...
def candidate_regis(root):
    ...
    r2.bn = PhotoImage(file="elements\\next1.png")
    btn = Button(r2, image=r2.bn, bg='#FFFFFF', bd=0,
                 activebackground="#ffffff", command=lambda: candidate_check(root))
    btn.place(x=320, y=500)
...
def candidate_complete(root):
    ...
    r3.bn = PhotoImage(file="elements\\reg.png")
    btn = Button(r3, image=r3.bn, bg='#FFFFFF', bd=0,
                 activebackground="#ffffff", command=lambda: candidate_submit(root))
    btn.place(x=320, y=550)


def candidate_submit(root):
    ...
    if gender1 and age1 and loc1 and workxp1:
        exe = f'insert into users values("{name1}","{email1}","candidate","{pwd1}")'
        exe1 = f'INSERT INTO mydb.Candidate(CID, CName , CEmail, CAge, CLocation, CGender, CExp, CSkills, CQualification ) VALUES (NULL, "{name1}", "{email1}", {age1}, "{loc1}", "{gender1}", {workxp1}, "{skills1}", "{qualification1}");'
        try:
            mycon = sql.connect(host='localhost', user='root',
                                passwd=user_pwd, database='mydb')
            cur = mycon.cursor()
